...
class PresensiController extends Controller
{
    ...
    {
        try {
            // 1. Catat ke log bahwa fungsi WA mulai dijalankan
            \Illuminate\Support\Facades\Log::info("=== MULAI PROSES KIRIM WA ===");
            \Illuminate\Support\Facades\Log::info("Status Fonnte Enable: " . ($this->fonnteService->isEnabled() ? 'AKTIF' : 'MATI'));

            if (!$this->fonnteService->isEnabled()) {
                \Illuminate\Support\Facades\Log::warning("Batal kirim: Toggle Fonnte belum aktif atau device tidak ada.");
                return;
            }

            // 2. Eksekusi kirim pesan
            $result = $this->fonnteService->sendPresensiNotification($presensi, $type);
            
            // 3. Catat hasil balasan dari Fonnte
            \Illuminate\Support\Facades\Log::info("Respon Fonnte: ", $result);

            if ($result['success']) {
                // Saya matikan sementara update status ke DB untuk mencegah error SQL 
                // jika kolom 'notifikasi_checkin_terkirim' belum Anda buat di tabel presensi
                \Illuminate\Support\Facades\Log::info("WA BERHASIL TERKIRIM KE ORTU!");
            }

        } catch (\Exception $e) {
            // 4. Jika ada error coding/SQL, catat di log
            \Illuminate\Support\Facades\Log::error("ERROR CRASH WA: " . $e->getMessage());
        }
    }
