...
class FonnteService
{
    ...
    public function sendMessage(string $phoneNumber, string $message, ?FonnteDevice $device = null, ?int $kelasId = null): array
    {
        ...
        try {
            ...
            // Cegah pengiriman jika nomor tujuan blank setelah dinormalisasi
            if (empty($phoneNumber)) {
                return [
                    'success' => false,
                    'message' => 'Invalid target phone number'
                ];
            }
            ...
            // Gunakan format standar HTTP Json & bersihkan Token dari spasi yang tidak terlihat
            $response = Http::timeout(30)
                ->withHeaders([
                    'Authorization' => trim($device->api_key),
                ])
  ...
  public function sendPresensiNotification($presensi, string $type = 'checkin'): array
    {
        ...
        try {
            ...
            // Pastikan parent_phone valid
            $cleanPhone = preg_replace('/[^0-9]/', '', $siswa->parent_phone);
            if (empty($cleanPhone) || strlen($cleanPhone) < 9) {
                return [
                    'success' => false,
                    'message' => 'No valid parent phone number available',
                    'skipped' => true
                ];
            }
            ...
            // ==========================================
            // INI ADALAH VARIABEL YANG HILANG TERSEBUT
            // ==========================================
            $variables = [
            ...
            // Kode ini yang tadi error karena tidak menemukan $variables di atas
    ...
    {
        // ... (Kode default template Anda tidak berubah)
        if ($type === 'checkin') {
